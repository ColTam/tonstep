...
QCPRange QCPDataContainer<DataType>::keyRange(bool &foundRange, QCP::SignDomain signDomain)
{
  ...
  {
    if (DataType::sortKeyIsMainKey()) // if DataType is sorted by main key (e.g. , but not QCPCurve), use faster algorithm by finding just first and last key with non-NaN value
    {
